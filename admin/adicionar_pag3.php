<?php
	...
	// ADICIONAR PAGAMENTO À TABELA DE PAGAMENTOS
		...
		$sql="INSERT INTO `sisoda_pagamentos`(`sisoda_pagamentos_idAluno`, `sisoda_pagamentos_valor`, `sisoda_pagamentos_paga`, `sisoda_pagamentos_data`,`sisoda_pagamentos_obs`) VALUES ('$id_aluno','$valor','0','$hoje','$obs')";
		$objDb = new db();
	    $link = $objDb->conecta_mysql();
	...
	// CHECAR SE ALGUMA AULA PODE SER PAGA
    	...
    	if ($resultado_id8) {
    		...
    		while($dados_login8 = mysqli_fetch_array($resultado_id8, MYSQLI_ASSOC)){
    		    ...
    		    $valor_aula=$dados_login8['sisoda_aulas_valorAluno'];
	...
	// CHECAR SE ALGUMA PENDENCIA PODE SER PAGA
	   ...
	   // ATUALIZAR SALDO DE TODOS OS ALUNOS
                    ...
                    if ($resultado_id15) {
                        ...
                        echo "<script>alert('Pagamento adicionado e o resultado foi: $aulas_pagas aulas pagas, $pend_pagas pendencias pagas, $mens_pagas mensalidades pagas. Fazendo que o novo saldo do aluno seja: R$ $saldo_final_aluno'); window.history.back()</script>";
